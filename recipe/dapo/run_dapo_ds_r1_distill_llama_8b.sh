...
# training params should hopefully fit in 50% of H100-80GB given it's 8b model.
gpu_memory_utilization=0.50
rollout_disable_log_stats=False # per verl/docs/perf/perf_tuning.rst
enable_chunked_prefill=False # prompts are short
tensor_model_parallel_size=1 # 8b model doesn't need TP
...
project_name='dapo'
exp_name='ds-r1-distill-llama-8b-exp-1'

PROJECT_DIR=${PROJECT_DIR:-"/workspace/verl"}
MODEL_PATH=${MODEL_PATH:-"/models/ds-r1-distill-llama-8b"}
...
# - fsdp offload params: False (unnecessary for 8b model)
